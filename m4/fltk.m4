AC_DEFUN([AC_CHECK_FLTK], [
  ...
  *** contains fltk-config.
      ...
      if test "x$FLTK_API_VERSION" = "x1.1" ||
         test "x$FLTK_API_VERSION" = "x1.3" ||
         test "x$FLTK_API_VERSION" = "x1.4"; then
          HAVE_FLTK_API_VERSION=yes
      fi
      COMPTEXT_FLTK_API_MAJOR=${FLTK_API_VERSION%%.*}
      COMPTEXT_FLTK_API_MINOR=${FLTK_API_VERSION#*.}; COMPTEXT_FLTK_API_MINOR=${COMPTEXT_FLTK_API_MINOR%%.*}
  ...
  *** To build $PACKAGE you need a FLTK version that provides API 1.1 or 1.3.
  ...
  AC_DEFINE_UNQUOTED([COMPTEXT_FLTK_API_MAJOR], [$COMPTEXT_FLTK_API_MAJOR], [FLTK API major version])
  AC_DEFINE_UNQUOTED([COMPTEXT_FLTK_API_MINOR], [$COMPTEXT_FLTK_API_MINOR], [FLTK API minor version])
